...
static class ObjLoaderFunctions {

public:

	static void Cross(float v1[3], float v2[3], float vout[3])
	{
		float tmp[3];
	...
	static float Unit(float vin[3], float vout[3])
	{
		float dist;
	...
	static char * ReadRestOfLine(FILE *fp)
	{
		static char *line;
	...
	static void ReadObjVTN(char *str, int *v, int *t, int *n)
	{
	...
	static Keyframe* LoadObjFile(char *name)
	{
		Keyframe *keyframe = new Keyframe(); //the keyframe that will be returned
			...
			if (strcmp(cmd, "f") == 0)
			{
				...
				for (int it = 0; it < numTriangles; it++)
				{
					...
					for (int vtx = 0; vtx < 3; vtx++)
					{
						...
						keyframe->AddVertex(x, y, z, nx, ny, nz);
					}
				}
				continue;
			}
